    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="px-4 pb-12">
        <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={feature.label} className="flex flex-col items-center gap-2 p-4 rounded-xl bg-white border border-gray-100 shadow-sm">
              <feature.icon className="h-6 w-6 text-indigo-500" />
              <span className="text-sm text-gray-600 text-center">{feature.label}</span>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <Link href="/employer" className="group">
              <Card className="h-full bg-white border border-gray-200 hover:border-indigo-300 hover:shadow-md transition-all duration-300 overflow-hidden">
                <CardHeader className="pb-4">
                  <div className="flex items-center gap-4">
                    <div className="h-14 w-14 rounded-xl bg-gray-100 flex items-center justify-center group-hover:bg-indigo-50 transition-colors">
                      <Building2 className="h-7 w-7 text-gray-500 group-hover:text-indigo-500 transition-colors" />
                    </div>
                    <div>
                      <CardTitle className="text-xl text-gray-900">사장님용 대시보드</CardTitle>
                      <CardDescription className="text-gray-400">Employer Dashboard</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="space-y-4">
                  <ul className="space-y-2 text-gray-600">
                    {["실시간 매칭 현황 확인", "신뢰 기반 인력 관리", "간편 공고 등록", "큰 글씨, 직관적인 UI"].map(item => (
            ...
            <Link href="/worker" className="group">
              <Card className="h-full bg-gradient-to-br from-indigo-600 to-violet-700 border-0 hover:from-indigo-500 hover:to-violet-600 transition-all duration-300 hover:shadow-lg hover:shadow-indigo-200 overflow-hidden">
                <CardHeader className="pb-4">
                  <div className="flex items-center gap-4">
      ...
      <footer className="px-4 py-8 border-t border-gray-100">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-gray-400 text-sm">일손매칭 - 제조업 일용직 매칭 플랫폼</p>
        </div>
      </footer>
    </div>
  )
